...
class Simulation {
    constructor(dimensions, initialConditions, mechanism, gridSize = GRID_SIZE) {
        ...
        for (var step of this.mechanism) {
            for (var species of step.reactants) {
                this.speciesInvolved.add(species);
            }
            for (var species of step.products) {
                this.speciesInvolved.add(species);
            }
        }
        for (let i = 0; i < initialConditions.length; i++) {
            var num = initialConditions[i][1];
            var species = initialConditions[i][0];
            for (let j = 0; j < num; j++) {
    ...
    addParticle(species,
        ...
        var p = new Particle(species, pos, vel);
        this.particleList.push(p);
        this.fixPos(p);
        var thisGridX = Math.floor(p.pos.x / this.gridSize);
        var thisGridY = Math.floor(p.pos.y / this.gridSize);
    ...
    genEmptyGrid() {
        var numGridsX = Math.ceil(this.dimensions.x / this.gridSize) + 1;
        var numGridsY = Math.ceil(this.dimensions.y / this.gridSize) + 1;
        var emptyGrid = new Array(numGridsX);
        for (let i = 0; i < numGridsX; i++) {
            var row = new Array(numGridsY);
    ...
    tick(amount = 1) {
        for (var n = 0; n < amount; n++) {
            var toAdd = [];
            var newGrid = this.genEmptyGrid();
            for (var gridRow of this.particleGrid) {
                for (var gridSquare of gridRow) {
                    for (var i = 0; i < gridSquare.length; i++) {
                        ...
                        for (var j = i + 1; j < gridSquare.length; j++) {
                            var p2 = gridSquare[j];
                            ...
                            if (p.isColliding(p2)) {
                                ...
                                for (var step of this.mechanism) {
                                    if ((step.reactants[0] === p.species && step.reactants[1] === p2.species) ||
                                        ...
                                        if (p.getKE() + p2.getKE() >= step.eA) {
                                            ...
                                            var totalEnergy = p.getKE() + p2.getKE() - step.deltaH;
                                            var toEach = totalEnergy / step.products.length;
                                            var angle = p.vel.getAngle();
                                            for (var product of step.products) {
                                                var vel = vectorFromAngle(angle, Math.sqrt(2 * toEach / product.mass));
                                ...
                                if (!reacted) {
                                    var deltaX = p.pos.difference(p2.pos);
                                    var deltaV = p.vel.difference(p2.vel);
                                    var m1, m2;
                                    m1 = p.species.mass;
                                    m2 = p2.species.mass;

                                    var deltaXMag = deltaX.mag();
                                    var resolveIntersectionDelta = deltaX.getScaled(((p.radius + p2.radius) - deltaXMag) / deltaXMag);
                                    p.pos.add(resolveIntersectionDelta.getScaled(0.5));
                                    p2.pos.add(resolveIntersectionDelta.getScaled(-0.5));
                                    var deltaV1 = deltaX.getScaled((2 * m2 * dotProduct(deltaV, deltaX)) / ((m1 + m2) * deltaX.magSq()));
                                    deltaX.scale(-1);
                                    deltaV.scale(-1);
                                    var deltaV2 = deltaX.getScaled((2 * m1 * dotProduct(deltaV, deltaX)) / ((m1 + m2) * deltaX.magSq()));
                        ...
                        this.fixPos(p);
                        if (!p.removed) {
                            var newGridX = Math.floor(p.pos.x / this.gridSize);
                            var newGridY = Math.floor(p.pos.y / this.gridSize);
                            try {
    ...
    getComposition() {
        var comp = {};
        for (var species of this.speciesInvolved) {
            comp[species.name] = 0;
        }
        for (var p of this.particleList) {
            comp[p.species.name]++;
...
var getParticleID = makeCounter();
var frameCount = makeCounter();

var a = new Species("A", "Red", 5);
var b = new Species("B", "Blue", 5);
var c = new Species("C", "Yellow", 15);
var d = new Species("D", "Green", 20);

var e = new MechanismStep([a, b], [c], 5, 0);
var f = new MechanismStep([c, a], [d], 200, 0);
...
function init() {
    var dimensions = new Vector(window.innerWidth, window.innerHeight);
    var mechanism = [e, f];
    var s = new Simulation(dimensions, INITIAL_COMP, mechanism);
    initGraph("graph", s);
    graphics(s, "canvas");
}
...
function graphics(sim, canvas, drawGrid = false) {
    ...
    canvas.width = sim.dimensions.x;
    canvas.height = sim.dimensions.y;
    ...
    function draw() {
        ...
        if (drawGrid) {
            ctx.fillStyle = "Black";
            for (var x = 0; x < Math.ceil(sim.dimensions.x / sim.gridSize); x++) {
                ctx.fillRect(x * sim.gridSize, 0, 2, sim.dimensions.y);
            }
            for (var y = 0; y < Math.ceil(sim.dimensions.y / sim.gridSize); y++) {
                ctx.fillRect(0, y * sim.gridSize, sim.dimensions.x, 2);
            }
        }
        for (var p of sim.particleList) {
            ctx.beginPath();
